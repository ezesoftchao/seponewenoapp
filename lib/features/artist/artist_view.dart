...
import 'package:seponewenoapp/features/artist/widgets/topBarArtist.dart';
...
class _ArtistsViewState extends State<ArtistView> {
  ...
  void getData() async {
    ...
    final response = await http.get('https://www.seponeweno.nat.cu/artistas');
      ...
      name = elements
          .map((element) => element
              .getElementsByClassName(
                  "button play-icon popup-with-zoom-anim")[0]
              .innerHtml)
          .toList();
                ...
                child: Column(
                  children: <Widget>[
                    Container(
                        ...
                        child: topAppBarArtist('Artistas')),
                    SizedBox(
                      height: 15.0,
                    ),
                    name.length == 0
                        ? Center(
                            ...
                            "Cargando",
                            style: TextStyle(
                                color: Colors.white54, fontSize: 32.0),
                          ))
                        : TextField(
                            onChanged: (value) {
                              setState(() {});
                          ...
                          return InkWell(
                            onTap: () {
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                    builder: (context) => SongsView(
                                        'https://www.seponeweno.nat.cu/' +
                                            link[index],
                                        'https://www.seponeweno.nat.cu/' +
                                            image[index])),
                              );
