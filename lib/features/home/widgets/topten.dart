...
class TopTen extends StatefulWidget {
  @override
  TopTenState createState() => TopTenState();
}
...
class TopTenState extends State<TopTen> {
  ...
  void getDataArtist() async {
    ...
    final top10 = document.querySelectorAll('div.contentLinksTopTen');
    setState(() {
      top10link = top10
          .map((element) =>
  ...
  Widget build(BuildContext context) {
    return top10song.length == 0 ? Center(child: Text("Cargando...", style: TextStyle( color: Colors.white54, fontSize: 45 ),),) :
    Row(
      children: <Widget>[
        Expanded(
            child: SizedBox(
                                            ...
                                            children: <Widget>[
                                              Container(
                                                child: OutlineButtonClass(
                                                    ...
                                                    "SePoneWeno",
                                                    "Top Actual",
                                                    "https://img.eldefinido.cl/portadas/650/2013-03-22-1054KI6449.jpg" )
                                              ),
                                              Container(
                                                child: OutlineButtonDownload(top10song[index], top10link[index])
                                              ),
                                            ],
                                          ),
                                          SizedBox(height: 15),
  ...
  void showToast3(BuildContext context, obj) {
    final scaffold = Scaffold.of(context);
    scaffold.showSnackBar(
      SnackBar(
        content: Text('Descargado - '+_fileName),
        action: SnackBarAction(
            label: 'Abrir', onPressed: (){
